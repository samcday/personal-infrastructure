...
if ! hcloud load-balancer describe cluster >/dev/null 2>&1; then
  hcloud load-balancer create --algorithm-type round_robin --location $location --name cluster --type lb11
fi
...
for name in cluster{1..3}; do
  if ! hcloud server describe $name >/dev/null 2>&1; then
    hcloud server create --name $name --type $server_type --placement-group cluster --ssh-key $ssh_key --image $image --location $location
    ssh-keygen -R "$(hcloud server ip $name)"
  fi

  server_id=$(hcloud server describe $name -o "format={{.ID}}")

  if [[ -z "$(hcloud load-balancer describe cluster -o json | jq ".targets | map(select(.server.id == $server_id)) | .[]")" ]]; then
    hcloud load-balancer add-target cluster --server $name
  fi
done
...
for name in cluster{1..3}; do
  ...
  result=$(ssh -T root@"$server_ip" <<-"INIT"
    ...
    [[ -f /etc/kubernetes/admin.conf ]] && echo "yes" || echo "no"
INIT
)
  if [[ "$result" == "yes" ]]; then
    cluster_created=$name
  fi
done
...
if [[ -z "$cluster_created" ]]; then
  ...
  cluster_created=cluster1
fi

if ! kubectl config get-contexts | grep personal-admin@personal >/dev/null 2>&1; then
  mkdir -p ~/.kube
  hcloud server ssh $cluster_created 'cat /etc/kubernetes/admin.conf | sed -e s/kubernetes-admin/personal-admin/' > ~/.kube/personal.conf
...
discovery_token_hash=sha256:$(hcloud server ssh $cluster_created -T "openssl x509 -pubkey -in /etc/kubernetes/pki/ca.crt | openssl rsa -pubin -outform der 2>/dev/null | openssl dgst -sha256 -hex | sed 's/^.* //'")
join_token=$(hcloud server ssh $cluster_created -T <<"TOKEN"
...
cert_key=$(hcloud server ssh $cluster_created -T <<"CERTS"
  set -ueo pipefail
...
if ! kubectl --context=personal-admin@personal -n kube-system get deploy/cilium-operator >/dev/null 2>&1; then
  cilium install --context=personal-admin@personal
fi
